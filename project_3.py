...
conn = psycopg2.connect(host = 'localhost',
                        ...
                        password = 'Kizor@1996',
                        port = 5432,
                        database = 'BusinessCard_extraction')
cursor = conn.cursor()
...
with col2:
                ...
                if path:                
                    ...
                    with col4:
                        if extract:
                            ...
                            try:
                                query = f"SELECT email FROM BusinessCard_extraction WHERE email = '{image_details['email']}';"
                                ...
                                li = df.values.tolist()
                                query = "insert into BusinessCard_extraction values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    ...
    if menu == 'Database':
        query = "SELECT * FROM BusinessCard_extraction;"
        cursor.execute(query)
        conn.commit()
        df = pd.DataFrame(cursor.fetchall(), columns=[desc[0] for desc in cursor.description])
        st.header("Database")                    
