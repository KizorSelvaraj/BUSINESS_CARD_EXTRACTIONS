...
with st.sidebar:    
    selected = option_menu("Main menu", ["Home", "Upload"], 
        ...
        if selected1 =="UPDATE":
            ...
            if update:
                ...
                qr='''update bs set name =%s,designation =%s,company_name =%s,contact =%s,alternative =%s,
                        email =%s,website =%s,street =%s,city =%s,state =%s,pincode =%s where contact =%s'''
...
#:HOME:
if selected=="Home":
    st.subheader(":red[Welcome to business card application]")
    st.subheader(":green[IMPORTANCE OF OCR]")
    ...
    image=("C:\\Users\\KizorSelvaraj\\Desktop\\project_3\\easyocr_framework.jpg")
